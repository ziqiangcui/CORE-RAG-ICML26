...
def get_custom_reward_fn(config):
    '''该函数从配置中读取自定义奖励函数的路径和名称，动态加载该模块，并返回一个包装后的奖励函数'''
    import importlib.util
    import sys

    reward_fn_config = config.get("custom_reward_function") or {}
    file_path = reward_fn_config.get("path")
    if not file_path:   # 如果路径为空，直接返回None，有路径才会考虑名称
        return None
...
def load_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    ...
    elif reward_manager_name == "apiprime":
        from verl.workers.reward_manager import ApiPrimeRewardManager

        reward_manager_cls = ApiPrimeRewardManager
    else:
